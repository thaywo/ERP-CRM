      ...
      if (request()->ajax()) {
        return datatables()->of(Invoice::with('project:id,title')
            ...
            ->addColumn('action', function ($data) {
              $button = '<a id="' . $data->id . '" class="show btn btn-success btn-sm" href="' . route('invoice.show', $data) . '"><i class="dripicons-preview"></i></a>';
              $button .= '&nbsp;&nbsp;';
              return $button;
            })
            ->rawColumns(['action'])
    ...
    if ($logged_user->role_users_id == 3) {
      if (request()->ajax()) {
        return datatables()->of($a)
            ...
            ->addColumn('action', function ($data) {
              $button = '<button type="button" name="show" id="' . $data->id . '" class="show btn btn-success btn-sm"><a href="' . route('invoice.show', $data) . '"><i class="dripicons-preview"></i></a></button>';
              $button .= '&nbsp;&nbsp;';
              return $button;
            })
            ->rawColumns(['action'])
